...
export class ProtocolDecoder {
    ...
    message = { type: 0, seq: 0, payload: new Uint8Array(0), valid: false };
    processByte(byte) {
        switch (this.state) {
            case 'WAIT_HEADER_0':
            ...
            case 'WAIT_PAYLOAD':
                if (this.message.payload.length < this.expectedPayloadLen) {
                    ...
                    console.warn(`[ProtocolDecoder] ⚠️ Payload complete (${this.message.payload.length}/${this.expectedPayloadLen}) but still in WAIT_PAYLOAD`);
                    console.warn(`[ProtocolDecoder] 🔍 Attempting recovery: treating byte 0x${byte.toString(16)} as CRC low, bufferLen=${this.buffer.length}`);
                    // Process this byte as CRC low byte
                    this.buffer.push(byte);
            ...
            case 'WAIT_CRC_1':
                ...
                if (this.validateFrame()) {
                    ...
                    console.log(`[ProtocolDecoder] ✅ Frame decoded successfully: type=0x${this.message.type.toString(16)}, seq=${this.message.seq}, payloadLen=${this.message.payload.length}`);
                    this.reset();
                    return true;
                }
                else {
